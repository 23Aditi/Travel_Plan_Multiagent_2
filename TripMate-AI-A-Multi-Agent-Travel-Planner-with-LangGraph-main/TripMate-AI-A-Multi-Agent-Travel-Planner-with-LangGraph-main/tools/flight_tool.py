...
def search_flights(query: str, limit: int = 10):
    cache_key = f"flights:{query.strip().lower()}:{limit}"
    ...
    if not flight_data:
        ...
        tool_cache.set(cache_key, res, ttl=1800)
        return res
    ...
    final_result = f"{route_info}\n\n" + "\n\n---\n\n".join(formatted_flights)
    tool_cache.set(cache_key, final_result, ttl=3600)
    return final_result
